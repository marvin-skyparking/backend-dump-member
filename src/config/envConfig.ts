...
if (EnvConfig.NODE_ENV == 'production') {
  ...
  EnvConfig.DB_PROD = EnvConfig.DB_PROD ?? 'db_skyccc';
  EnvConfig.ENABLE_SWAGGER = EnvConfig.ENABLE_SWAGGER ?? true;
  EnvConfig.PORT = EnvConfig.PORT ?? 9000;
  EnvConfig.USERNAME_DB_PROD = EnvConfig.USERNAME_DB_PROD ?? 'root';
  EnvConfig.PASSWORD_DB_PROD = EnvConfig.PASSWORD_DB_PROD ?? '50p4y5ky0v0!';
